...
class MemoryGraph:
    def __init__(self, config):
        ...
        parts = self.config.graph_store.config.url.split(":")
        ...
        ok = connection_pool.init([(parts[0], parts[1])], config)

        # get session from the pool
        self._session = connection_pool.get_session(self.config.graph_store.config.username,
                                              self.config.graph_store.config.password)

        self._session.execute(f"USE {self.config.graph_store.config.database}")
